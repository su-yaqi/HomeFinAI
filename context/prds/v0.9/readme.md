...
3. 完成交易写工具及统一确认、幂等机制。
4. 完成预算与分类写工具。
5. 在前四项全部通过验收后移除旧链路，并以同一个 v0.9 发布包完成切换。

Codex Desktop 是 v0.9 必测客户端：必须使用计划发布时的实际桌面版本完成连接、授权、Token 轮换、`input_required`、重启重连和撤销失效测试，不能只依赖协议单元测试。

开发过程中允许旧代码在分支内短暂存在，但 v0.9 对外发布状态不得同时提供两套同定位 AI 接入能力。
...
- 尚未完成计划发布版 Codex Desktop 的真实添加服务器、OAuth 回调、重启重连、确认拒绝/超时和撤销验收，因此 v0.9 不能标记为已发布完成。
