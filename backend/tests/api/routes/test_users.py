...
def test_create_user_new_email(
    ...
    with (
        ...
        assert created_user["login_name"] == username.split("@", 1)[0]
...
def test_read_handler_users_as_normal_user(
    client: TestClient,
    normal_user_token_headers: dict[str, str],
    db: Session,
) -> None:
    user = create_random_user(db)
    user.full_name = "Handler Candidate"
    db.add(user)
    db.commit()
    db.refresh(user)

    response = client.get(
        f"{settings.API_V1_STR}/users/handler-options",
        headers=normal_user_token_headers,
    )
...
def test_admin_can_reset_user_mfa_secret(
    ...
    assert r.status_code == 200
    db.refresh(user)
    assert r.json() == {"message": "MFA reset successfully"}
    assert user.mfa_secret is None


def test_user_can_setup_enable_and_disable_mfa(
    client: TestClient, normal_user_token_headers: dict[str, str], db: Session
) -> None:
    ...
    assert setup_payload["otpauth_uri"]
    ...
    enable_response = client.post(
        ...
        json={
            ...
            "code": _hotp(setup_payload["secret"], int(time.time() // 30)),
        },
    )
    ...
    assert current_user_response.status_code == 200
    assert current_user_response.json()["has_mfa"] is True

    current_user = db.get(User, current_user_id)
    assert current_user is not None
    assert current_user.mfa_secret == setup_payload["secret"]
...
def test_user_can_reset_their_own_mfa(
    client: TestClient, normal_user_token_headers: dict[str, str], db: Session
) -> None:
    current_user_response = client.get(
        f"{settings.API_V1_STR}/users/me",
    ...
    assert current_user_response.status_code == 200
    user = db.get(User, current_user_response.json()["id"])
    ...
    reset_response = client.post(
        f"{settings.API_V1_STR}/users/me/mfa/reset",
        headers=normal_user_token_headers,
    )
    assert reset_response.status_code == 200
    assert reset_response.json() == {"message": "MFA reset successfully"}

    db.refresh(user)
    assert user.mfa_secret is None


def test_enable_mfa_with_invalid_secret_returns_400(
    client: TestClient, normal_user_token_headers: dict[str, str]
) -> None:
