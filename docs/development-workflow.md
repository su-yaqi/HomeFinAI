...
`BASE_REF...HEAD`。本地声明只记录交付证据，不会被误当成 CI 已通过。

## 5. 分级 CI 与单一门禁

自动化入口为 `.github/workflows/ci.yml`，稳定 required check 名称是
`CI / CI Required`。仓库分支保护只需要求这一项；门禁会汇总所有本次实际选择的作业，
任何已选择作业失败都会使门禁失败。

- PR：`scripts/ci-plan.sh` 根据变更路径推断最低 S/M/H 风险并选择后端、前端、E2E、
  Compose 和制品校验。纯 S 文档变更不会运行无关 E2E。
- `main`、夜间计划和手动完整运行：无条件执行完整检查。
- H 路径：无条件选择后端、前端、E2E、Compose 和制品校验，不能用路径过滤缩减。
- 制品校验：检查冻结锁文件、生成客户端无漂移、Alembic 迁移头、开发/内网 Compose
  渲染、固定内网端口和备份服务配置。

`make test-ci-plan` 回归路径选择规则，`make validate-compose` 校验开发与正式内网
Compose 的静态语义。历史独立测试工作流保留为手动诊断入口，不再分别触发 PR/main，
也不再作为多个 required checks。
